...
def naver_news_crawlling():
    print('시작합니당')
    client_hdfs = InsecureClient("http://192.168.236.135:9870", user="hadoop")
            ...
            for jsonIdx in res.json()['items']:
                if 'n.news.naver' in jsonIdx['link']:
                    ...
                    if len(content) > 0:
                        ...
                        client_hdfs.write(f'/user/hadoop/wordcount_test/{filename}.txt', data=value, overwrite=True, encoding="UTF8")
